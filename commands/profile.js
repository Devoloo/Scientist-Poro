...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('profile')
        .setDescription('Get League of Legends profile (you can also search for a linked Discord account).')
	...
	async execute(interaction) {
        ...
        const link = await DiscordRiot.findOne({ where: { discordId: discordId } });

        if (!link) {
            if (me) 
                return interaction.reply({ content: 'You have not linked your account.', ephemeral: true });
            return interaction.reply({ content: 'This account is not linked.', ephemeral: true });
        }
        ...
        try {
            summoner = await rAPI.summoner.getByAccountId({
                region: PlatformId.EUW1,
                accountId: link.riotId,
            });
        } catch (error) {
